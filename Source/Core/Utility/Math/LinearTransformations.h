...
namespace sp {
	...
	inline Matrix4x4 const getLookAtMatrix(Vector3 const & forward, Vector3 const & up, Vector3 const & position) {
		Matrix4x4 translateMatrix = translate(Matrix4x4{}, -position);
		Matrix4x4 coordinateSystemMatrix = getCoordinateSystemMatrix(up, forward);

		return coordinateSystemMatrix * translateMatrix;
	}
